...
sed -i '$ s/$/tmux new-session -s Lampac -d "proot-distro login debian -- bash /root/lampac_updater.sh"/' .bashrc
else
...
sed -i '$ s/$/tmux new-session -s Jackett -d "proot-distro login debian -- /home/Jackett/./jackett"/' .bashrc
else
...
sed -i '$ s/$/tmux new-session -s Torrserver -d "proot-distro login debian -- /home/torrserver/torrserver -p 8091"/' .bashrc
else
